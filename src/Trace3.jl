...
const live_die      = Condition() # statement on the human condition?

function trace{T}(dep::Response{T},
    ...
    # Create loop variables to hold the values to source to the stimuli.
    localvars = [gensym() for i in length(indep)]

    # Preallocate output memory. We want an array in column-major order.
    array = Array{T}(reverse([length(a) for (i,a) in indep])...)

    # Describe looping over an independent variable...
    loops = [
        (expr)->quote
            let x = $(localvars)[$i], stim = $(indep)[$i][1], itr = $(indep)[$i][2]
                for x in enumerate(itr)
                    source(stim, x)
                    $expr
                end
            end
        end for i in length(indep)]

    # Describe the body of the trace loop
    body = quote
        block(dep, indep...)
        array[reverse(localvars)...] = measure(dep)
    end
