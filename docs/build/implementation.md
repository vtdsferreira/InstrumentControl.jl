...
## Code organization
  ...
  * Each instrument is defined within its own module, a submodule of `PainterQB`. Each instrument is a subtype of `Instrument` or `InstrumentVISA`. By convention, instrument model numbers are used for module definitions (e.g. `AWG5014C`), so type names have "Ins" prepended (e.g. `InsAWG5014`).
